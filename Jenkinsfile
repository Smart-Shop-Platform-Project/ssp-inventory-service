    ...
    stages {
        ...
        stage('Build and Push Docker Image') {
            steps {
                script {
                    dir('terraform') {
                        sh "terraform init -backend-config=\"bucket=ssp-terraform-state-bucket\" -backend-config=\"key=services/inventory-service/terraform.tfstate\" -backend-config=\"region=${AWS_REGION}\""
