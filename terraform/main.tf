...
resource "aws_iam_policy" "ssm_policy" {
  name        = "ssp-inventory-service-ssm-policy-${var.environment}"
  description = "Allows reading specific SSM parameters for the inventory service"
    ...
    Statement = [
      {
        ...
        Effect   = "Allow",
        Resource = "arn:aws:ssm:${var.aws_region}:${data.aws_caller_identity.current.account_id}:parameter/${var.kafka_broker_url_param_name}"
      }
    ]
  })
}
...
module "ecs_service" {
  ...
  environment_variables = [
    { name = "KAFKA_BROKER_URL_PARAM_NAME", value = var.kafka_broker_url_param_name },
